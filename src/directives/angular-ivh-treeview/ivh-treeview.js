...
angular.module('ivh.treeview').directive('ivhTreeview', ['ivhTreeviewMgr', function(ivhTreeviewMgr) {
  ...
  return {
    ...
    controller: ['$scope', '$element', '$attrs', '$transclude', 'ivhTreeviewOptions', 'filterFilter', function($scope, $element, $attrs, $transclude, ivhTreeviewOptions, filterFilter) {
      ...
      ng.forEach([
        'childrenAttribute',
         'defaultSelectedState',        'expandToDepth',
        'idAttribute',
      ...
      trvw.aa = function(node) {
         if(node.isSelected1==true){
           node.isSelected1=false;
         }else{
           node.isSelected1=true;
         }
      };
   ...
   /* twistieCollapsedTpl: '(+)',*/
    ...
    nodeTpl: [
      '<div class="ivh-treeview-node-content" title="{{trvw.label(node)}}">',
        ...
        '</span>',
        '<span ng-if="node.unitCodefgbm==null&&node.zzjz==null" class="ivh-treeview-checkbox-wrapper" ng-if="trvw.useCheckboxes()"',
            'ivh-treeview-checkbox>',
        '</span>',
        '<span ng-if="node.unitCodefgbm!=null&&node.zzjz!=5" class="ivh-treeview-checkbox-wrapper" ng-if="trvw.useCheckboxes()"',
        'ivh-treeview-checkbox>',
        '</span>',
        ...
        '> ',
      '<input ng-if="node.unitCodefgbm!=null&&node.zzjz!=5" style="background-color:#69DEEA;margin:-6px 0 0 "   ng-checked="node.isSelected1" type="radio"  class="ivh-treeview-checkbox" ng-model="isSelected1"   ng-click="trvw.aa(node)" />' ,
      '</span>',
