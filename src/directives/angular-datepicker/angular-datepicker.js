...
//Moment format filter.
  Module.filter('mFormat', function () {
    return function (m, format, tz) {
      // modify by zk @2016-3-31 if m is null , do not return today's string
      if (!m) return m;
  ...
  Module.directive('datePicker', ['datePickerConfig', 'datePickerUtils', '$filter', function datePickerDirective(datePickerConfig, datePickerUtils, $filter) {
    ...
    return {
      ...
      link: function (scope, element, attrs, ngModel) {
        ...
        // FIXME 先这样设置值吧，不要是date要字符串
        var dateFilter = $filter('mFormat');
        setDate = function (date) {
          if (date) {
            scope.model = dateFilter(date, 'YYYY-MM-DD HH:mm:ss'); //date;

            if (ngModel) {
              ngModel.$setViewValue(dateFilter(date, 'YYYY-MM-DD HH:mm:ss'));
            }
          }
  ...
  Module.constant('dateTimeConfig', {
    template: function (attrs, id) {
      return '' +
        ...
        'class="date-picker-date-time" fx-autoscroll></div>';
    },
    ...
    return {
      ...
      link: function (scope, element, attrs, ngModel) {
        var format = attrs.format || dateTimeConfig.format,
          ...
          dismiss = true, //attrs.autoClose ? $parse(attrs.autoClose)(scope) : dateTimeConfig.autoClose,
          picker = null,
          pickerID = element[0].id,
        ...
        function updateInput(event) {
          ...
          //为解决跟投 出资 ，跟投现金流 动态表格中的 日期控件 onChange 特殊处理
          //2017-2-9 添加,调用指定的 事件处理函数，完成 日期onChange回调功能
          if(event.currentScope && event.currentScope.$parent && event.currentScope.$parent.vm){
        	  if(event.currentScope.$parent.gtUser && event.currentScope.$parent.vm.dateChangeEvent){
        		  event.currentScope.$parent.vm.dateChangeEvent(event.currentScope.$parent.gtUser);
        	  }
          }
        }
        ...
        function showPicker() {
          ...
          if (!shownOnce) {
            scope.$on('setDate', function (event, date, view) {
              ...
              if (dismiss && minView === view /*views[views.length - 1] === view*/) {
                clear();
              }
            });
