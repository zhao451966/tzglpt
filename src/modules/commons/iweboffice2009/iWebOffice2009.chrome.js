...
(function() {
    ...
    angular.module('demo.core')
        .controller('IWebOfficeChromeController', IWebOfficeChromeController);

    function IWebOfficeChromeController($scope,Restangular,FileUtils, Confirm, toastr,file,fileAccessAPI,Authenticate ) {
    	...
    	//作用：上传本地文件到服务器
    	window.WebPutFile = vm.WebPutFile = function (saveName) {
    		var mLocalFile = webform.WebOffice.WebOpenLocalDialog();
    		if (mLocalFile != "") {
    			//交互OfficeServer的OPTION="PUTFILE"  参数1表示本地路径，可以任何格式文件  参数2表示服务器文件名称
    			if (webform.WebOffice.WebPutFile(mLocalFile, saveName)) {
    				window.StatusMsg(webform.WebOffice.Status);
    			} else {
    				window.StatusMsg(webform.WebOffice.Status);
    			}
    		}
...
//    			WebOffice.WebSetProtect(true,"");//保护文档状态
    	...
    	window.OnMenuClick = vm.OnMenuClick =function (vIndex, vCaption) {
    		try {
				//响应菜单事件
//				if ("1" == vIndex) {
//					//上传文件
//					var LocalFile = webform.WebOffice.WebOpenLocalDialog();
//					if (LocalFile) {
//						webform.WebOffice.WebPutFile(LocalFile,"uploadFile.temp");
//					}
//				} else if ("2" == vIndex) {
//					//打开本地文件
...
//						var n = WordObject.Application.ActiveDocument.PageSetup.DifferentFirstPageHeaderFooter;
//						if(n == undefined){
//							if(webform.WebOffice.EditType.charAt(0)=="0"){
//								Confirm("您没有编辑文档权限，无法添加水印！", {title: '提示',okText:'关闭',noCancel:true});
//							}else{
//								Confirm("由于您的Office版本兼容性，无法添加水印！", {title: '提示',okText:'关闭',noCancel:true});
//							}
//						}else if (n == "9999999") { // 含有分节符的文档
//							WordObject.Application.ActiveWindow.ActivePane.View.SeekView = 2;
//							window.WaterMark(WordObject, vm.markTxt); // 添加文字水印
//							WordObject.Application.ActiveDocument.Sections(3).Range
//									.Select();
...
//						}
//					} else if (webform.WebOffice.FileType == ".xls") {
//						//					var sheetsCount = WordObject.Application.Worksheets.Count;
//						//					for(var i=1;i<=sheetsCount;i++){
//						//						WordObject.Application.Worksheets(i).Activate;
//						//						WordObject.Application.Worksheets.ActiveSheet.Shapes.AddTextEffect(0, vm.markTxt, "宋体", 1, false, false, 0,0).Select();
//						//					}
//					} else if (webform.WebOffice.FileType == ".ppt") {
//
//					}
...
//				} else if ("5" == vIndex) {
//					//保存到服务器
//					webform.WebOffice.WebUpdateFile(true);
//					var fileMap = eval(webform.WebOffice.WebGetMsgByName("FILEMAP"));
//					alert(fileMap);
//				} else if ("6" == vIndex) {
//					//关闭文件
//					webform.WebOffice.WebClose();
...
//				}
				...
				webform.WebOffice.Alert(e);
			}
    	};
    	
    	//工具栏事件
    	window.OnToolsClick = vm.OnToolsClick = function (vIndex, vCaption) {
    	};
    	
    	window.OnExecuteScripted = vm.OnExecuteScripted =function (name, result) { //可实现其他业务功能
    	}
    	...
    	//客户端功能：传递信息并接收服务器回传的信息 Demo
//    	function sendMessage(UserName,PassWord){
//    	  WebOffice.WebSetMsgByName("COMMAND","GETDEPARTMENT");
//    	  WebOffice.WebSetMsgByName("_USERNAME",UserName);
//    	  WebOffice.WebSetMsgByName("_PASSWORD",PassWord);
//    	  if (WebOffice.WebSendMessage()){
//    	    alert(WebOffice.WebGetMsgByName("_DEPARTMENT"));
//    	    alert(WebOffice.WebGetMsgByName("_COMPANY"));
//    	  }
//    	}
    	//作用：打开远程文件,将远程文件写到本 ，再从本地打开 
//    	window.WebDownLoadFile = vm.WebDownLoadFile =function (url,localpath) {
//    		mResult = webform.WebOffice.WebDownLoadFile(
//    				"http://localhost/src/modules/commons/iweboffice2009/test.doc",
//    				"c:\\abc.doc");
//    		if (mResult) {
...
//    	};
    	...
    	vm.showHelp=function(){
    		window.open('modules/commons/iweboffice2009/iWebOffice.help.html');
    	};
