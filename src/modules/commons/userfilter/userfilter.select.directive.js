(function() {
    ...
    angular.module('demo.core')
        .directive('fxUserfilterSelect', UserfilterSelect);
    ...
    function UserfilterSelect() {
        return {
            restrict: 'AE',
            replace: true,
            templateUrl: 'modules/commons/userfilter/userfilter.select.directive.html',
            controller: UserfilterSelectCtrl
        };
    }

    /* @ngInject */
    function UserfilterSelectCtrl($scope, FilterAPI,$translate) {
        ...
        $scope.$queryFilter = function(id) {
            FilterAPI.one('list').one($scope.$$model).getList()
                .then(function(data) {
                    ...
                    // 添加默认的全部信息
                    data.unshift({
                        filterName: $translate.instant('N000997'),//N000997=全部查看,
                        selected: !id
                    });
