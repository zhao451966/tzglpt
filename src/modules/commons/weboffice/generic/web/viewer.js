...
/**
 * 说明
 * 1.国际化 语言设置 请查找本文代码： mozL10n.setLanguage(PDFJS.locale);
 * 2.控件初始化方法 请查找本文代码： function webViewerInitialized() {
 * 3.语言 转换 请查找本文代码  ： 
 * if(window.I18N=='zh_CN'){
	  PDFJS.locale='zh-CN';
   }else if(window.I18N=='en_US'){
	  PDFJS.locale='en-US';
   }
 */

/** 默认notFoundPdf  */
var DEFAULT_URL = 'notFoundPdf.pdf';
...
/** 1分半钟默认超时 */
var DISABLE_AUTO_FETCH_LOADING_BAR_TIMEOUT = 90000;
var timeOutCloseFirstRun = false;//标识超时定时器 是否是第一次执行
/** 根据服务器返回的 带权限的文件路径 请求pdf */
if(!window.parent.fileUrl){
//	window.parent.getPdf();
//	if(!window.parent.fileUrl ){
//		DEFAULT_URL = '/fxtgpt/service/weboffice?filepath='+encodeURIComponent(window.parent.fileUrl);
//	}
//	window.parent.fileUrl='';
}else{
	DEFAULT_URL = ContextPath+'service/weboffice?fileId='+encodeURIComponent(window.parent.fileUrl);
	window.parent.fileUrl='';
}
 ...
 * 下载原文件
 */
var downloadOldDoc = function(){
	if(DEFAULT_URL != 'notFoundPdf.pdf'){
		window.parent.getOldDoc();
	}
}
/**
 * 删除原文件
 */
var deleteOldDoc = function(){
	/**删除源文件*/
	window.parent.deleteFile();
//	/**设置默认notFoundPdf*/
//	DEFAULT_URL = 'notFoundPdf.pdf';
//	/**查看器 重新初始化*/
...
function watchScroll(viewAreaElement, callback) {
  ...
  var scrollTop_val =0;
  if(viewAreaElement && viewAreaElement.scrollTop){
	  scrollTop_val=viewAreaElement.scrollTop;
  }
  var state = {
    down: true,
    lastY: scrollTop_val,
...
var DEFAULT_PREFERENCES = {
  ...
  timeOutClose:true,//超时自动关闭遮罩层给提示
...
var DELAY_BEFORE_RESETTING_SWITCH_IN_PROGRESS = 1500; // in ms 复位开关演示时间
var DELAY_BEFORE_HIDING_CONTROLS = 3000; // in ms 隐藏控件延时时间
...
var PDFPresentationMode = (function PDFPresentationModeClosure() {
  ...
  PDFPresentationMode.prototype = {
    ...
    /**
     * 用于初始化请求显示模式时的超时，
     *这个时间是用来防止当前页面的滚动 部分，或完全，在进入演示模式时的视图。当文档加载完成，用户才可以滚动页面
...
var PDFViewer = (function pdfViewer() {
  ...
  function PDFViewer(options) {
    ...
    if(!options.viewer){
    	if(options.container && options.container.firstElementChild){
    		this.viewer = options.container.firstElementChild;
    	}
    }
  ...
  PDFSidebar.prototype = {
    ...
    _addEventListeners: function PDFSidebar_addEventListeners() {
      ...
      self.attachmentsView.addEventListener('attachmentsloaded', function(evt) {
    	//解锁页面
    	  var pageLock = document.getElementById("FullPageLock");
    	  pageLock.setAttribute('style', 'display: none;');
...
function webViewerInitialized() {
  ...
  //超时自动隐藏遮罩层
  if(PDFViewerApplication.timeOutClose){
	  PDFJS.timeOutClose=true;
  }
...
//    SecondaryToolbar.downloadClick.bind(SecondaryToolbar));
  ...
  //显示loading 图片
  var pageLock = document.getElementById("FullPageLock");
  pageLock.setAttribute('style', 'display: block;z-index: 10000;position: relative;height: 100%;top:25px;');
    ...
    xhr.onreadystatechange = function() {  
        ...
        if(xhr.status == 504) {
        	...
        	pageLock.setAttribute('style', 'display: block;z-index: 10000;position: relative;height: 100%;top:25px;');
      	  	...
      	  	document.getElementById("textMsg").setAttribute('style','color:red;size: 20px;');
      	  	document.getElementById("textMsg").innerText ="time out..";
        }
    };  
    xhr.onload = function() {
...
function webViewerLoad() {
  ...
  /** 按钮权限控制 **/
  if(window.parent.filePermision){
  	//deleteOldDoc downloadOldDoc print
  	if(!window.parent.filePermision.view){
  		var downBtn = document.getElementById('print');
  		downBtn.setAttribute('style', 'display: none;');

  	}
  	if(!window.parent.filePermision.down){
  		var downBtn = document.getElementById('downloadOldDoc');
  		downBtn.setAttribute('style', 'display: none;');
  	}
  	if(!window.parent.filePermision.remove){
  		var removeBtn = document.getElementById('deleteOldDoc');
  		removeBtn.setAttribute('style', 'display: none;');
  	}
  }
  /** 打开office按钮控制 **/
  if(window.parent.isOffice){
	  var officeBtn = document.getElementById('openOffice');
	  officeBtn.setAttribute('style', 'display: block;');
  }
  //显示loading 图片
  var pageLock = document.getElementById("FullPageLock");
  pageLock.setAttribute('style', 'display: block;z-index: 10000;position: relative;height: 100%;top:25px;');
  ...
  //5秒中后显示  提示文本
  setTimeout(function(){
	   var tr = document.getElementById("msgTr");
	   tr.setAttribute('style','visibility: visible;text-align: left;margin: 0px auto;width: 240px;');
//	   document.getElementById("loadingTr").setAttribute('style','visibility: hidden;');
   },5000);
