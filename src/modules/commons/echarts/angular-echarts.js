...
function getLinkFunction($http, theme, util, type, $timeout) {
    return function (scope, element, attrs) {
        ...
        function getSizes(config) {
            ndParent = element.parent()[0], parentWidth = ndParent.clientWidth, parentHeight = ndParent.clientHeight;
        ...
        // 为了解决tab切换时dom还未显示时就执行指令，造成无法获取父容器宽度和高度bug
        ...
        angular.forEach(data, function (serie) {
            ...
            angular.forEach(serie.datapoints, function (datapoint) {
                if(config.special === 'bar'){
                    var data={
                        value : datapoint.y>=0?datapoint.y:-datapoint.y,
                        tooltip:{
                            trigger: 'item',
                            ...
                            formatter: function () {
                                return serie.name + '<br/>' + datapoint.x + ' : ' + datapoint.y;
                            }
            ...
            if (isPieChart(type)) {
                ...
                // different serie needs different config. modify by zk
                conf.center = config.center || serie.center || ['40%', '50%'];
                conf.radius = config.radius || serie.radius || '60%';
 ...
 */
angular.module('angular-echarts.theme', []).factory('theme', ['infographic','firstPage', 'shinePie','macarons', 'shine', 'dark', 'blue', 'green', 'red', function (infographic,firstPage,shinePie, macarons, shine, dark, blue, green, red, grey) {
    var themes = {
 ...
 * 首页主题 theme
 */
angular.module('angular-echarts.theme').factory('firstPage', function () {
    return {
        // 默认色板
        color: [
                    '#2ec7c9','#b6a2de','#5ab1ef','#ffb980','#d87a80',
 ...
 * 首页饼图专用
 */
angular.module('angular-echarts.theme').factory('shinePie', function () {
    return {
        // 默认色板
        color: [
                    '#c12e34','#e6b600','#0098d9','#2b821d',
