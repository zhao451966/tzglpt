(function() {
    ...
    angular.module('demo.core')
        .controller('DialogUnitByUserController', DialogUnitByUserController);

    /* @ngInject */
    function DialogUnitByUserController($scope, $timeout, Restangular,selected) {
        ...
        vm.buildParam=function(type){
        	var param={
        			//后端都按照多部门查询
        			s_type:"init-multiple",
        			s_unitCodes:selected
        			};
         ...
         * 根据部门名称查询
         */
        vm.searchUnitByName=function(byName){
        	if($scope.ngDialogData.userCode){
        		var param = vm.buildParam();
				Restangular.all('unitinfo/listUnit')
				.customGETLIST('',param)
				.then(function(data) {
					if(data){
						vm.units = data;
			            vm.units.forEach(function(obj) {
			            	if($scope.ngDialogData.userSelectedUnitCode && obj.unitCode== $scope.ngDialogData.userSelectedUnitCode ){
			            		obj.isSelected=true;
