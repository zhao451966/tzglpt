(function() {
    ...
    angular.module('demo.core')
        .controller('DialogUserEmailController', DialogUserEmailController);

    /* @ngInject */
    function DialogUserEmailController($scope, $timeout, Restangular, selected,UserInfoAPI) {
        var vm = this;
        ...
        vm.buildParam=function(type){
        	...
        	if(type=="init-single"){
        		...
        		param.s_userNameOrEmail = vm.name;
        	}
        ...
        vm.searchUserByName=function(){
            ...
            if(selected){
            	if($scope.ngDialogData.selectionMode == 'single'){
            		...
            		//查询一个用户
            		Restangular.all('userinfo/findUserEmailInfoList')
                    .customGETLIST('',param)
                    .then(function(data) {
						if(data){
                    		vm.users = data;
            	...
            	var param = vm.buildParam("userNameOrEmail");
            	Restangular.all('userinfo/findUserEmailInfoList')
                .customGETLIST('',param)
                .then(function(data) {
                    vm.users = data;
