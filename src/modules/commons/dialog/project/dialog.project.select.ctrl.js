(function(){
    ...
    angular
        .module('demo.core')
        .controller('DialogProjectSelectController',DialogProjectSelectController);

    /* @ngInject */
    function DialogProjectSelectController($scope,  $timeout,Restangular, selected,Authenticate){
        var vm = this;
        ...
        vm.query = function() {
			...
			if(selected){
				if($scope.ngDialogData.selectionMode == 'single'){
					...
					//查询一个用户
					Restangular.all('projects/listProject')
						.customGETLIST('',param)
						.then(function(data) {
							if(data){
								vm.projects = data;
