...
(function(){
	...
	var Provider, aFilters = [], nIdCount = 0, aOtherBrowsers = ["Maxthon", "SE 2.X", "QQBrowser"],
		...
		sTotalContainer = '<div id="#totalContainerId#" class="stream-total-tips">' +
			...
			'</div>',
		sFilesContainer	= '<div class="stream-files-scroll" style="height:20px;"><ul id="filesContainerId"></ul></div>';
	...
	var StreamUploader = function(){
		...
		this.config = {
			...
			uploadURL : "",
			serverAddress : "",
			portionSize : 1048576,//原先10M=10485760，现改为5M=5242880 2M=2097152 1M=1048576
			parameters : {},
			fileFieldName : "FileData",
	...
	StreamUploader.prototype = {
		...
		initializer: function(file){
			...
			this.file = null;
			this.fileId = null;
			this.filePiece = 1048576;//原先10M=10485760，现改为5M=5242880 2M=2097152 1M=1048576
	...
	Main.prototype = {
		...
		upload : function(index) {
			...
			if(this.config.urlParam && this.config.urlParam!="undefined"){
				uploadURL = uploadURL+"&"+this.config.urlParam ;
			}
		...
		uploadError : function(evt) {
			...
			// 2016-9-24 添加，解决上传失败后 在选文件无法上传，页面一直等待
			this.cancel();//
		},
		...
		validateFile : function(uploader) {
			...
			/*最大上传2G文件*/
			if(!bStreaming && size > 2147483648){this.uploadError({status:100, statusText:"最大只能上传2G的文件!"});return !1;}
			if(size ==0){
				/*如果文件大小=0 空文件不许上传*/
				this.get("onMaxSizeExceed") ? this.get("onMaxSizeExceed")(info) : this.onMaxSizeExceed(info);
			}else if (this.get("maxSize") < size){
				/*文件超出 配置的最大长度*/
				this.get("onMaxSizeExceed") ? this.get("onMaxSizeExceed")(info) : this.onMaxSizeExceed(info);
			}else {
				/*校验文件后缀*/
	...
	var bStreaming = function() {
		...
		//alert("bFormData----" + bFormData + "---Browser.os----" + Browser.os + "---Browser.safari---" + Browser.safari);
		(bFormData && Browser.os === "windows" && Browser.safari === "5.1.7") && (bFormData = !1);
		//alert("bFile----" + bFile + "---bFormData---" + bFormData + "---bHtml5---" + bHtml5);
